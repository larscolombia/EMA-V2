...
class FakeProfileService implements ProfileService {
  ...
  @override
  Future<UserModel> updateProfileImage(UserModel profile, imageFile) async =>
      profile;
}
...
class FakeChatsService extends ChatsService {
  ...
  Future<ChatModel> generateNewChat(
    ...
    if (!Get.isRegistered<ActionsService>()) {
  ...
  @override
  Future<ChatStartResponse> startChat(String prompt) async =>
      ChatStartResponse(threadId: 'threadX', text: '');
  @override
  Future<ChatMessageModel> sendMessage({
    required String threadId,
...
Future<void> _registerBaseDeps({
  FakeChatsService? chats,
  ProfileControllerCounters? profile,
}) async {
...
void main() {
  ...
  test('forceStopAndReset agrega burbuja temporal y limpia estados', () async {
    ...
    c.isSending.value = true;
    c.isTyping.value = true;
    c.forceStopAndReset();
  ...
  test('Persistencia de threadId vía SharedPreferences', () async {
    ...
    SharedPreferences.setMockInitialValues({
      'current_thread_id': 'persistido123',
    });
    await _registerBaseDeps();
    final c = Get.put(ChatController());
  ...
  test('PDF only en hilo existente asigna prompt por defecto', () async {
    ...
    expect(
      lastUser.text.toLowerCase().contains('analiza el archivo adjunto'),
      true,
    );
  ...
  test(
    'Retry con PDF ya no decrementa cuota en cliente (server-side only)',
    () async {
      await _registerBaseDeps();
      if (Get.isRegistered<ChatsService>()) {
        Get.delete<ChatsService>();
      ...
      await c.sendMessage(
        'Falla',
      ); // en este flujo falla después de intentar validar/enviar; puede haber decremento si llegó a éxito parcial
  ...
  test('Detección de stuck state resetea isSending', () async {
    ...
    c.isSending.value = true;
    c.lastSendTime.value = DateTime.now().subtract(const Duration(seconds: 61));
    await c.sendMessage(
      'Ignorado',
    ); // debería invocar _checkForStuckState y resetear
    expect(c.isSending.value, false);
  });
}
