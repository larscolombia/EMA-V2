...
class ClinicalCasesServices {
  ...
  Future<ClinicalCaseGenerateData> generateCase(
      ClinicalCaseModel temporalCase) async {
  ...
  Future<List<QuestionResponseModel>> loadQuestionsByCaseId(String caseId) async {
  ...
  Future<List<ChatMessageModel>> loadMessageByCaseId(String caseId) async {
    ...
    final items = await _chatMessagesLocalData.getItems(where: where, whereArgs: whereArgs);

    return items;
  }

  Future<QuestionResponseModel> _updateAnswer(QuestionResponseModel question) async {
    final where = 'id = ?';
  ...
  Future<QuestionResponseModel> sendAnswer(QuestionResponseModel questionWithMessage) async {
    ...
    final feedBackAndNewQuestion = await _apiClinicalCaseData.sendAnswerMessage(questionWithMessage);
  ...
  Future<List<ChatMessageModel>> startAnalytical(ClinicalCaseModel clinicalCase) async {
    try {
      // String generatePrompt = ClinicalCaseHelper.promptStartAnalytical();
      // generatePrompt += 'Caso clínico enviado al usuario para que lo analice:';
      // generatePrompt += clinicalCase.toPrompt();

      const generatePrompt = 'Genera una pregunta abierta sobre este caso clínico.';
      final userMessage = ChatMessageModel.user(chatId: clinicalCase.uid, text: generatePrompt);
      ...
      final aiFirsQuestions = await _apiClinicalCaseData.sendMessage(userMessage);
  ...
  Future<QuestionResponseModel> startInteractive(ClinicalCaseModel clinicalCase) async {
    ...
    if (aiFirsQuestion == null) {
      ...
      aiFirsQuestion = await _apiClinicalCaseData.sendAnswerMessage(userFirstMessage);
    }
