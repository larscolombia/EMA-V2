...
class ApiUserTestProgressService extends UserTestProgressService {
  ...
  Future<Map<String, dynamic>> _doFetchOverview({
    ...
    if (response.statusCode == 200) {
      ...
      try {
        ...
        if (active != null) {
          final plan =
              (active['subscription_plan'] as Map<String, dynamic>?)?['name'];
          ...
          // ignore: avoid_print
          print(
            '[QUOTA] plan=$plan consultations=$cons questionnaires=$quest clinical_cases=$clin files=$files',
          );
        }
      } catch (_) {}
      return data;
  ...
  Future<TestProgressData> fetchTestScores({
    ...
    return TestProgressData.fromJson({
      'data': {'tests': recentTests, 'resumen': summary},
    });
  }
