...
func (m *mockAI) StreamMessage(ctx context.Context, prompt string) (<-chan string, error) { ch := make(chan string,1); m.lastPrompt = prompt; ch <- "ok"; close(ch); return ch,nil }
func (m *mockAI) StreamAssistantMessage(ctx context.Context, threadID, prompt string) (<-chan string, error) { ch := make(chan string,1); m.lastPrompt = prompt; ch <- "ok"; close(ch); return ch,nil }
func (m *mockAI) EnsureVectorStore(ctx context.Context, threadID string) (string, error) { return "vs_mock", nil }
func (m *mockAI) UploadAssistantFile(ctx context.Context, threadID, filePath string) (string, error) { return "file_mock", nil }
func (m *mockAI) PollFileProcessed(ctx context.Context, fileID string, timeout time.Duration) error { return nil }
...
func (m *mockAI) StreamAssistantMessageWithFile(ctx context.Context, threadID, prompt, filePath string) (<-chan string, error) { return m.StreamAssistantMessage(ctx, threadID, prompt) }
