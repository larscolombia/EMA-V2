...
func (h *Handler) evaluateLastAnswer(threadID, userAnswer string, explicit *int, data map[string]any) (bool, bool) {
	if threadID == "" { return false, false }
	...
	if _, pending := data["evaluation_pending"].(bool); !pending {
		...
		if isCorrect { h.evalCorrect[threadID] = h.evalCorrect[threadID] + 1 }
	}
	h.mu.Unlock()
...
func (h *Handler) StartCase(c *gin.Context) {
	...
	if h.lastCorrectIndex == nil { h.lastCorrectIndex = make(map[string]int) }
	if h.lastOptions == nil { h.lastOptions = make(map[string][]string) }
	if h.lastQuestionText == nil { h.lastQuestionText = make(map[string]string) }
	if h.missingCorrectIdx == nil { h.missingCorrectIdx = make(map[string]int) }
	if h.closureDue == nil { h.closureDue = make(map[string]bool) }
	...
	if threadID != "" {
		...
		if nx, ok := data["next"].(map[string]any); ok {
			if pq, ok := nx["pregunta"].(map[string]any); ok {
				...
				if txt, ok := pq["texto"].(string); ok { h.lastQuestionText[threadID] = strings.TrimSpace(txt) }
				// extract options to support answer matching
				if rawOpts, ok := pq["opciones"].([]any); ok {
					var opts []string
					for _, v := range rawOpts { if s, ok := v.(string); ok { opts = append(opts, strings.TrimSpace(s)) } }
					if len(opts) > 0 { h.lastOptions[threadID] = opts }
				} else if rawStr, ok := pq["opciones"].([]string); ok {
					var opts []string
					for _, s := range rawStr { opts = append(opts, strings.TrimSpace(s)) }
					if len(opts) > 0 { h.lastOptions[threadID] = opts }
				}
			}
		}
...
func (h *Handler) Message(c *gin.Context) {
	...
	if closing {
		...
		if len(prevQs) > 5 { prevQs = prevQs[len(prevQs)-5:] }
		prevList := ""
		if len(prevQs) > 0 {
	...
	if !closing {
		...
		if threadID != "" {
			...
			if (!okCI || ci < 0 || ci >= len(opts)) && len(opts) > 0 {
				...
				h.mu.Lock(); h.missingCorrectIdx[threadID] = h.missingCorrectIdx[threadID] + 1; h.mu.Unlock()
				if recIdx, ok := h.recoverCorrectIndex(ctx, threadID, qText, opts); ok {
					h.mu.Lock(); h.lastCorrectIndex[threadID] = recIdx; ci = recIdx; okCI = true; h.mu.Unlock()
				}
			}
			if okCI && len(opts) > 0 && ci >= 0 && ci < len(opts) { _, _ = h.evaluateLastAnswer(threadID, req.Mensaje, req.AnswerIndex, data) } else { data["evaluation_pending"] = true }
		}
		rebuildIfEmbedded := func(preg map[string]any) {
			...
			var existing []string
			for _, v := range rawOpts { if s, ok := v.(string); ok { existing = append(existing, strings.TrimSpace(s)) } }
			...
			if len(extracted) >= 2 {
				...
				for _, o := range extracted { arr = append(arr, o) }
				preg["opciones"] = arr
			}
		}
		...
		if !h.closureDue[threadID] { h.incrementCount(threadID) }
		...
		case cnt == maxQuestions:
			data["finish"] = 0.0
			h.mu.Lock(); h.closureDue[threadID] = true; h.mu.Unlock()
		default:
			data["finish"] = 0.0
		}
	} else {
		...
		// limpiar flag
		h.mu.Lock(); delete(h.closureDue, threadID); h.mu.Unlock()
	}
	...
	if threadID != "" {
		...
		if v, ok := data["evaluation_pending"].(bool); ok && v { evalObj["pending"] = true; evalObj["is_correct"] = nil }
...
func forceFinishInteractive(data map[string]any, threadID string, h *Handler) {
	...
	if fbOriginal != "" {
		...
		for _, ln := range lines {
			...
			if strings.HasPrefix(ul, "EVALUACIÓN:") || strings.HasPrefix(ul, "EVALUACION:") { continue }
			cleaned = append(cleaned, ln)
		}
		fbOriginal = strings.Join(cleaned, "\n")
	}
	coreSummary := extractCoreSummary(fbOriginal)
	if coreSummary == "" { coreSummary = "No disponible" }
	...
	h.mu.Lock(); miss := h.missingCorrectIdx[threadID]; h.mu.Unlock()
...
func performanceTier(pct float64) string {
	switch {
	case pct >= 85: return "Excelente"
	case pct >= 70: return "Bueno"
	case pct >= 50: return "Aceptable"
	case pct > 0: return "Necesita refuerzo"
	default: return "Sin aciertos"
	}
}
...
func mapUserAnswerToIndex(userRaw string, explicit *int, options []string) (int, bool) {
	if explicit != nil && options != nil && *explicit >=0 && *explicit < len(options) { return *explicit, true }
	...
	if len(u)==1 {
		...
		switch {
		case c>='A' && c<='Z':
			idx := int(c-'A'); if idx < len(options) { return idx,true }
		case c>='a' && c<='z':
			idx := int(c-'a'); if idx < len(options) { return idx,true }
		case c>='0' && c<='9':
			idx := int(c-'0'); if idx < len(options) { return idx,true }
		}
	}
	...
	if bestIdx >=0 && bestScore >= 0.4 { return bestIdx,true }
	return -1,false
}
...
func extractCoreSummary(fb string) string {
	...
	for _, ln := range lines {
		...
		if len(kept) >= 2 { break } // keep it brief
	}
...
func rebuildFeedbackWithEvaluation(original string, isCorrect bool) string {
	...
	if isCorrect { eval = "Evaluación: CORRECTO" }
...
func normalizeAnswer(s string) string {
	...
	for _, r := range runes {
		...
		switch r {
 		case 'á','à','ä','â','Á','À','Ä','Â': r = 'a'
 		case 'é','è','ë','ê','É','È','Ë','Ê': r = 'e'
 		case 'í','ì','ï','î','Í','Ì','Ï','Î': r = 'i'
 		case 'ó','ò','ö','ô','Ó','Ò','Ö','Ô': r = 'o'
 		case 'ú','ù','ü','û','Ú','Ù','Ü','Û': r = 'u'
 		case 'ñ','Ñ': r = 'n'
 		}
...
func jaccard(a, b []string) float64 {
	...
	for _, w := range a { setA[w]=struct{}{} }
	for _, w := range b { setB[w]=struct{}{} }
	var inter int
	for w := range setA { if _, ok := setB[w]; ok { inter++ } }
...
func stripLeadingLetterPrefix(s string) string {
 	...
 	if len(trim) >=3 {
 		...
 		if (c>='A' && c<='D') || (c>='a' && c<='d') {
 			...
 			if strings.HasPrefix(rest, "-") || strings.HasPrefix(rest, ")") { return strings.TrimSpace(rest[1:]) }
 		}
 	}
 	return trim
}
...
func answerIsCorrect(userRaw, correctRaw string) bool {
	...
	if userClean == correctClean { return true }
 	...
 	if len(ut) >=2 {
 		...
 		// subset allowance
 		var subset = true
 		for _, w := range ut { if !containsToken(ct, w) { subset=false; break } }
 		if subset && float64(len(ut))/float64(len(ct)) >= 0.5 { return true }
 	}
 	// containment for long phrases
 	if len(userClean) >= 15 && strings.Contains(correctClean, userClean) { return true }
 	return false
}

func containsToken(tokens []string, w string) bool { for _, t := range tokens { if t==w { return true } }; return false }
...
func (h *Handler) recoverCorrectIndex(ctx context.Context, threadID, question string, options []string) (int, bool) {
	if threadID == "" || len(options) == 0 { return -1, false }
	// construir prompt compacto enumerando opciones 0..n
	var sb strings.Builder
	sb.WriteString("Pregunta previa: \"")
	sb.WriteString(question)
	sb.WriteString("\". Opciones:")
	for i, opt := range options { sb.WriteString(" "); sb.WriteString(strconv.Itoa(i)); sb.WriteString(") "); sb.WriteString(opt); sb.WriteString(";") }
...
func allShortLetters(opts []string) bool {
	if len(opts)==0 { return true }
	for _, o := range opts { oc := strings.TrimSpace(o); if len(oc) != 1 { return false }; if !( (oc[0]>='A'&&oc[0]<='D') || (oc[0]>='a'&&oc[0]<='d') ) { return false } }
	return true
}
...
func extractEmbeddedOptions(s string) []string {
	...
	for _, ln := range lines {
		...
		if len(m) == 3 {
			...
			if body == "" { continue }
			if _, ok := seen[body]; ok { continue }
			seen[body] = struct{}{}
			res = append(res, body)
		}
	}
	if len(res) >= 2 && len(res) <= 6 { return res }
	return nil
}
